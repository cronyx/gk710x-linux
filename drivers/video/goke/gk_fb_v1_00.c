...
/* video=gk_fb0:<x_res>x<y_res>,<x_virtual>x<y_virtual>,
   <color_format>,<conversion_buffer>[,<prealloc_start>,<prealloc_length>][,<convert_format>] */
	...
	struct gk_platform_fb *gk_fb_data)
{
    ...
    unsigned int                cl_convert_format;
	if (!options || !*options) {
		goto gkfb_setup_exit;
	}

	ssret = sscanf(options, "%dx%d,%dx%d,%d,%d,%x,%x,%x", &cl_xres, &cl_yres,
		&cl_xvirtual, &cl_yvirtual, &cl_format, &cl_cvs_buf,
		&cl_prealloc_start, &cl_prealloc_length, &cl_convert_format);
	if (ssret == 6) {
        ...
        gk_fb_data->color_format = cl_format;
        gk_fb_data->conversion_buf.available = cl_cvs_buf;
        gk_fb_data->convert_format = cl_prealloc_start;

        dev_dbg(dev, "%dx%d,%dx%d,%d,%d,%x\n", cl_xres, cl_yres,
            cl_xvirtual, cl_yvirtual, cl_format, cl_cvs_buf,
            cl_prealloc_start);
        retval = 0;
    } else if (ssret == 9) {
        ...
        gk_fb_data->screen_fix.smem_len = cl_prealloc_length;
        gk_fb_data->use_prealloc = 1;
        gk_fb_data->convert_format = cl_convert_format;
        dev_dbg(dev, "%dx%d,%dx%d,%d,%d,%x,%x,%x\n", cl_xres, cl_yres,
            cl_xvirtual, cl_yvirtual, cl_format, cl_cvs_buf,
            cl_prealloc_start, cl_prealloc_length, cl_convert_format);
        retval = 0;
    } else {
