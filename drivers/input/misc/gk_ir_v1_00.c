...
static void gk_ir_init(struct gk_ir_info *pirinfo)
{
    ...
    gk_ir_writel( REG_IRR_PROG, 0x07);  // 上升，下降沿有效，计数溢出中断，用于处理433 wifi
#elif defined(CONFIG_INPUT_GOKE_IR)
    gk_ir_writel( REG_IRR_PROG, 0x02);  // 上升沿有效，用于处理红外
#endif
    ...
    if(irq_flag & IRQ_EN_IRR) // irr irq
    {
        ...
        if (m_irr_read.bitc.valid == 1)
        {
                        ...
                        if(counting==50)
                        {
                                ...
                                && (IrBuffer[4] + IrBuffer[5]) < 1200)
                            {
                                ...
                                */
                                ...
                                {
                                    //printk("%d", IrBuffer[i]<400?1:0);  // 两个值判断一个bit位，最开始的两个数据是起始头的脉宽
                                                                        // 约定高电平短的为1，长的为0，短电平在200us左右，长电平
                                                                        // 在800us左右
                                    i+=2;
                                    i+=2;
                                ...
                                && (IrBuffer[4] + IrBuffer[5]) > 1600)
                            {
                                ...
                                */
                                ...
                                for(i=2;i<counting;i+=2)
                                {
                                    //printk("%d", IrBuffer[i]<900?1:0);  // 两个值判断一个bit位，最开始的两个数据是起始头的脉宽
                                                                        // 约定高电平短的为1，长的为0，短电平在500us左右，长电平
                                                                        // 在1500us左右
                                    i+=2;
            ...
            else
            {
                ...
                ts = ir_count2time(tmp_value);  //统计脉宽的时间(us)。不用时间，用脉冲计数也行

                IrBuffer[counting] = ts;
                if(counting >= IRR_MAX_DATA) counting = 0;
            }
#elif defined(CONFIG_INPUT_GOKE_IR)//下面是红外遥控器的按键处理
            if (tmp_value < 0xfC)
            {
