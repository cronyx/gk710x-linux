...
#if defined(CONFIG_SYSTEM_USE_NETLINK_MSG)
#include <net/sock.h>
#include <net/netlink.h>
#include <plat/gk_net_link.h>

struct sock *gk_nl_sk = NULL;
#endif
...
#if defined(CONFIG_SYSTEM_USE_NETLINK_MSG)
//----------------------------------------------------------------
void gk_nl_send_message(GK_NET_LINK_MSG_S* msg)
{
    struct sk_buff *skb;
    char *scratch;
    int len = sizeof(GK_NET_LINK_MSG_S);
    ...
    memcpy(scratch, msg, len);

    NETLINK_CB(skb).dst_group = 1;

    if(gk_nl_sk)
        netlink_broadcast(gk_nl_sk, skb, 0, 1, GFP_ATOMIC);
}
EXPORT_SYMBOL(gk_nl_send_message);

// Receive messages from netlink socket.
static void gk_nl_receive_message(struct sk_buff  *skb)
{
    printk("gk_nl_receive_message\n");
}
#endif
//----------------------------------------------------------------
...
#if defined(CONFIG_SYSTEM_USE_NETLINK_MSG)
    gk_nl_sk = netlink_kernel_create(&init_net, NETLINK_GK_IPC,
                        0, gk_nl_receive_message, NULL, THIS_MODULE);
    if (!gk_nl_sk)
    {
