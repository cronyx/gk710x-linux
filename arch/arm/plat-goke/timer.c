...
void set_ptz_timer_call_func(HIGH_RES_TIMER_CALLBACK func)
{
	s_pCallback_high_res = func;
}

EXPORT_SYMBOL(set_ptz_timer_call_func);

void set_timer_ms_delay_func(u32 time)
{
	static u32 old_time;
	old_time = timer_ms_delay;
	timer_ms_delay = time;
	timer_intr_cnt = 0;
}
EXPORT_SYMBOL(set_timer_ms_delay_func);

static void init_hw_timer(void)
{
    u32 interrupt_interval_in_ms = 1;
    u32 apb_clk = get_apb_bus_freq_hz();
    u32 timer_clk = apb_clk * interrupt_interval_in_ms /1000;
