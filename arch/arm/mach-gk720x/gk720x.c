...
#define GK_IO_DESC_HEVC_ID   6
#define GK_IO_DESC_RES_ID    7
...
static struct gk_mem_map_desc gk_mem_desc[] =
{
    ...
    {/*5*/
        ...
        .io_desc    =
        {
            .virtual= 0xff000000, // dsp_size <= 0xfe000000-0xf6000000
...
static u32 uboot_logo_base;
static u32 uboot_logo_pitch;
static u32 uboot_logo_size;
...
u32 get_hevcmem_phys(void)
{
    return __pfn_to_phys(gk_mem_desc[GK_IO_DESC_HEVC_ID].io_desc.pfn);
}
EXPORT_SYMBOL(get_hevcmem_phys);
u32 get_hevcmem_virt(void)
{
    return  gk_mem_desc[GK_IO_DESC_HEVC_ID].io_desc.virtual;
}
EXPORT_SYMBOL(get_hevcmem_virt);
u32 get_hevcmem_size(void)
{
    return gk_mem_desc[GK_IO_DESC_HEVC_ID].io_desc.length;
}
EXPORT_SYMBOL(get_hevcmem_size);
...
u32 get_logo_virt(void)
{
    return gk_phys_to_virt(uboot_logo_base);
}
EXPORT_SYMBOL(get_logo_virt);

u32 get_logo_pitch(void)
{
    return uboot_logo_pitch;
}
EXPORT_SYMBOL(get_logo_pitch);

u32 get_logo_size(void)
{
    return uboot_logo_size;
}
EXPORT_SYMBOL(get_logo_size);
...
void __init gk_map_io(void)
{
...
#if defined(CONFIG_ARCH_GK720X)|defined(CONFIG_ARCH_GK720X_FPGA)
    u32 hevc_phy_addr;
    u32 hevc_size;
    u32 res_phy_addr;
    u32 res_size;
...
#if 0//defined(CONFIG_ARCH_GK720X)|defined(CONFIG_ARCH_GK720X_FPGA)
    hevc_phy_addr = 0xc7e00000;
    hevc_size=0x200000;
    res_size=0x10000;
    usr_size = 0;
    usr_phy_addr = 0xC0000000 + mem_total - usr_size;
    bsb_phy_addr = 0xC0000000 + get_ppm_size() + mem_size;
	res_phy_addr = bsb_phy_addr + bsb_size;
    dsp_phy_addr = res_phy_addr+ res_size;
    dsp_size = mem_total - get_ppm_size() - mem_size - bsb_size - usr_size-hevc_size - res_size;
#else
    usr_size = 0x10000;
    usr_phy_addr = 0xC0000000 + mem_total - usr_size;
    bsb_phy_addr = 0xC0000000 + get_ppm_size() + mem_size;
...
#if 0//defined(CONFIG_ARCH_GK720X)|defined(CONFIG_ARCH_GK720X_FPGA)
    gk_mem_desc[GK_IO_DESC_RES_ID].io_desc.virtual = res_phy_addr;
    gk_mem_desc[GK_IO_DESC_RES_ID].io_desc.pfn = __phys_to_pfn(res_phy_addr);
    gk_mem_desc[GK_IO_DESC_RES_ID].io_desc.length = res_size;

    gk_mem_desc[GK_IO_DESC_HEVC_ID].io_desc.pfn = __phys_to_pfn(hevc_phy_addr);
	gk_mem_desc[GK_IO_DESC_HEVC_ID].io_desc.length = hevc_size;
#endif
    ...
    uboot_logo_base = *(volatile u32 *)CONFIG_U2K_BOOT_LOGO_ADDR;
    uboot_logo_pitch= *(volatile u32 *)CONFIG_U2K_BOOT_LOGO_PITCH;
    uboot_logo_size = *(volatile u32 *)CONFIG_U2K_BOOT_LOGO_SIZE;
